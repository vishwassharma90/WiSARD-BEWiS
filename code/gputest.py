...
#i have tensorflow in the system, the tensorflow environment is also there
...
def process_parallel(image,pos,index,dis):
...
#        total_sum_of_all_ram=[]
    for index,dis in enumerate(d):
        #sum_of_ram_output = process_parallel(image,pos,index,dis)
        p = Process(target=process_parallel, args=(image,pos,index,dis))     #,Lock
        p.start()
        p.join()
...
#    myarray = np.asarray(d, dtype = int)
#    print(type(d))
#    print(type(myarray))
    #print(d[0][0])
    #print(acc_pos)
